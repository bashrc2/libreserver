...
# Some fixes by Bob Mottram <bob@libreserver.org>
...
main() {
    ...
    if [ ! -x "$_file" ]; then
        printf '%s\n' "Cannot execute $_file (likely because of mounting /root/build/rustup as noexec)." 1>&2
...
get_architecture() {
    ...
    if [ $_ostype = "unknown-linux-gnueabihf" ]; then
        if [ $_cputype = armv7 ]; then
