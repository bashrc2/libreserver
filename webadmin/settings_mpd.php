...
$output_filename = "app_mpd.html";

if (php_sapi_name()!=='fpm-fcgi') exit('php script must be run from the web interface');

if (isset($_POST['submitfileformat'])) {
    if(filter_string('fileformat')) {
        $format = htmlspecialchars($_POST['fileformat']);
        $settings_file = fopen(".appsettings.txt", "w") or die("Unable to write to appsettings file");
        fwrite($settings_file, "mpd,fileformat,".$format);
