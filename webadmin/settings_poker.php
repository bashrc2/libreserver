...
$output_filename = "app_poker.html";

if (php_sapi_name()!=='fpm-fcgi') exit('php script must be run from the web interface');

if (isset($_POST['submitPoker'])) {
    $serverpassword = htmlspecialchars($_POST['serverpassword']);
    $settings_file = fopen(".appsettings.txt", "w") or die("Unable to write to appsettings file");
    fwrite($settings_file, "poker,serverpassword,".$serverpassword);
